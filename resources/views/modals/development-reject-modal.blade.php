<div class="show-reject-development-modal">
    <div class="reject-development-modal hidden fixed z-10 inset-0 overflow-y-auto text-center" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20  sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full {{ str_replace('_', '-', app()->getLocale()) === 'ar' ? 'text-right' : 'text-left' }}">
                <form method="POST" action="{{ route('admin.intsRejectTransfer') }}" enctype="multipart/form-data">
                    <input type="hidden" value="{{ $details_id }}" name="form_id" id="modal-action-form-id" />
                    <input type="hidden" name="reject_form" value="1" />
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex grow mx-auto items-center justify-center h-12 rounded-full bg-indigo-600 text-white">
                            ...
                            <h3 class="text-lg leading-6 font-medium" id="modal-title">
                                {{ trans('interventions.rejection-reason-modal') }}
                            </h3>
                        </div>
                        <div class="mt-3">
                            {{ csrf_field() }}
                            <div class="col-span-6 sm:col-span-3 py-2">
                                <label for="reject_notes" class=" text-sm font-medium text-gray-700">{{ trans('interventions.rejection-reason-modal') }}</label>
                                <textarea id="reject_notes" oninvalid="this.setCustomValidity('يجب تدوين سبب رفض الدعم')" oninput="this.setCustomValidity('')" pattern="[\u0621-\u064A\u0660-\u0669a-zA-Z ]{3,30}" name="reject_notes" rows="4" cols="50" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button type="button" class="close-reject-development-modal mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">{{ trans('modals.confirm_modal_button_cancel_text') }}</button>
                    </div>
                </form>
            </div>
        </div>
