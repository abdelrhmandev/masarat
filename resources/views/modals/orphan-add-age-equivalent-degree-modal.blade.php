<div class="show-orphan-add-age-equivalent-degree-modal">
    <div class="orphan-add-age-equivalent-degree-modal hidden fixed z-10 inset-0 overflow-y-auto text-center" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20  sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full {{ str_replace('_', '-', app()->getLocale()) === 'ar' ? 'text-right' : 'text-left' }}">
                <form method="POST" action="{{ route('admin.submit_orphan_age_equivalent_degree') }}" enctype="multipart/form-data">

                    <input type="hidden" name="form_id" id="action-form-id-age" />
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex grow mx-auto items-center justify-center h-12 rounded-full bg-indigo-600 text-white">
                            ...
                            <h3 class="text-lg leading-6 font-medium" id="modal-title">{{ trans('orphan.create_age_equivalent_degree') }}</h3>
                        </div>
                        <div class="mt-3">
                            {{ csrf_field() }}

                            <div class="col-span-6 sm:col-span-3 py-2">
                                <label for="orphan_age_equivalent_degree" class=" text-sm font-medium text-gray-700">{{ trans('orphan.age_equivalent_degree') }}</label>
                                <input name="orphan_age_equivalent_degree" type="number" oninvalid="this.setCustomValidity('يجب تدوين درجه العمر المكافيء')" oninput="this.setCustomValidity('')" pattern="^[0-9]*$" autocomplete="given-name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            </div>
                        </div>
                    </div>
